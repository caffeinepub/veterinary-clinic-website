      ...
      <div className="container mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            Protect your animals with our comprehensive vaccination programs.
        ...
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-4 mb-8 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Regular vaccination is the most cost-effective way to prevent
              infectious diseases in your herd, flock, or pets. Our
              veterinarians will help you create a customized vaccination
              schedule for any kind of animal.
            </p>
          </div>
        </div>
            ...
            {categoryConfig.map((config) => {
              ...
              return (
                <section key={config.category}>
                  {/* Category Header */}
                  <div
                    className={`flex items-center gap-4 p-5 rounded-xl border mb-4 ${config.headerClass}`}
                  >
                    <img
                      src={config.icon}
                      alt={config.title}
                    ...
                    <span
                      className={`ml-auto text-xs font-semibold px-2.5 py-1 rounded-full ${config.badgeClass}`}
                    >
                      {categoryVaccinations.length} vaccine
                      {categoryVaccinations.length !== 1 ? "s" : ""}
                    </span>
                  </div>
            ...
            {vaccinationServices.length === 0 && (
              <div className="text-center py-16">
                ...
                <h3 className="font-serif text-xl font-semibold text-foreground mb-2">
                  No vaccination programs listed yet
                </h3>
                <p className="text-muted-foreground">
                  Vaccination programs will appear here once added by the
                  clinic.
                </p>
              </div>
            )}
