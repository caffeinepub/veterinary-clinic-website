...
const categoryColors: Record<ServiceCategory, string> = {
  [ServiceCategory.largeRuminant]: 'bg-amber-100 text-amber-800 border-amber-200',
  ...
  [ServiceCategory.birdsPoultry]: 'bg-emerald-100 text-emerald-800 border-emerald-200',
};
    ...
    <Card className="flex flex-col h-full shadow-card hover:shadow-card-hover transition-shadow duration-200 border-border">
      <CardHeader className="pb-3">
        <div className="flex flex-wrap gap-1.5 mb-2">
          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${categoryColors[service.category]}`}>
            {categoryLabels[service.category]}
          </span>
          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${typeColors[service.serviceType]}`}>
            {typeLabels[service.serviceType]}
          </span>
        </div>
        <CardTitle className="font-serif text-lg leading-snug">{service.name}</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col flex-1 gap-3">
        <CardDescription className="text-sm leading-relaxed flex-1">{service.description}</CardDescription>
        <div className="pt-2 border-t border-border">
          <span className="text-xl font-bold text-primary font-sans">{formatPrice(service.price)}</span>
